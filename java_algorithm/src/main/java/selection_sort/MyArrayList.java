...
public class MyArrayList<T> implements List<T> {
    ...
    public boolean add(T element) {
        if (size >= array.length) {
            ...
            //System.arraycopy(Object arc, int srcPos, Object dest, int destPos, length)
            //src : 복사할 배열, srcPos : src에서 읽어올 위치, dest : 복사 대상 배열, destPos : dest 쪽에 복사할 시작 위치, length : src에서 읽어올 배열 요소의 수
    ...
    public T set(int index, T element) {
        T previewElement = get(index);  //get()에서 array 체크를 해주고 있기 때문이 따로 크기 체크 하지 않는다.
        array[index] = element;
        return previewElement;
    }

    private void arraySizeCheck(int index) {
        if (index < 0 || index >= size) {
            throw new IndexOutOfBoundsException();
        }
    }
